...
const observer = new MutationObserver(() => {
    // "button:has(div[data-a-selector])"
    const button = $('div:has(>button[aria-label="Clip Options"])');
    if (button) {
...
function setup() {
    ...
    button.onclick = () => {
        ...
        const name = `${$('a h1.tw-title').textContent} - ${$(
